...
サマリー指標も同じ境界で分ける。ゲストにはHome/探すでイベント数、ジム数、公開閲覧範囲だけを表示し、保護route直アクセスではログイン誘導を優先する。`今週の予定`、`保存ジム`、`記録` のような個人活動指標はログイン後に認証付きAPIから取得した値だけを表示する。ジム/イベント詳細では、ゲストに個人活動指標や関連予定/投稿を先出ししない。ログイン後の活動データ取得は画面単位で1回に集約し、Home、予定、記録、マイページ、サマリーが同じ取得結果を共有する。
