...
| `/`, `/home`, `/explore`, `/gyms`, `/events`, 公開投稿/公開予定/公開記録の詳細 | ゲスト閲覧可 |
| `/plans`, `/logs`, `/me`, `/plans/new`, `/logs/new`, `/posts/new`, 保存、参加、コメント、通報、通知、設定、マイページの個人データ | ログイン必須 |
| `/admin/*` | admin必須 |

guardはUIだけでなくAPI側でも必ず検証する。公開閲覧はanonymousを許可し、作成・保存・参加・コメント・個人設定はAPI側で認証を必須にする。
...
`/` はrouteを分けず、client側の認証状態で `GuestHomeExperience` と `MemberHomeExperience` を切り替える。ゲスト版は公開イベント、公開ショートカット、公開フィードを中心にし、ログイン済み版はメンバーダッシュボード、作成Composer、次の予定/保存ジム/最近の記録を中心にする。作成・保存・参加・コメントのAPI guardはこのUI分岐に依存せず維持する。

メインナビも認証状態で切り替える。ゲストは `ホーム` と `探す` のみ、ログイン済みは `予定`、`記録`、`マイ` を追加する。公開詳細ページはゲスト閲覧可だが、ゲストのトップレベルナビには載せない。
