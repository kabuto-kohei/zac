...
`/` はrouteを分けず、client側の認証状態で `GuestHomeExperience` と `MemberHomeExperience` を切り替える。ゲスト版はジム、イベント、探索ショートカットを中心にし、予定、記録、投稿は出さない。ログイン済み版はメンバーダッシュボード、作成Composer、次の予定/保存ジム/最近の記録を中心にする。作成・保存・参加・コメントのAPI guardはこのUI分岐に依存せず維持する。

メインナビも認証状態で切り替える。ゲストは `ホーム` と `探す` のみ、ログイン済みは `予定`、`記録`、`マイ` を追加する。ジム/イベント詳細はゲスト閲覧可だが、予定/記録/投稿の詳細はログイン後だけ表示する。
