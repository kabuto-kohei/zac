...
| 優先 | 画面 | route | 目的 |
|---|---|---|---|
| 1 | Home | `/` | ゲスト閲覧、ログイン誘導、公開ジム/イベント、次アクションを確認する |
...
UI方向性はZennのdark UIを参考にする。ただしZacのゲスト閲覧では、イベントとジムを「まず閲覧できる」情報として見せる。予定、記録、投稿はログイン後の活動面で扱う。
...
- 右: 認証状態で出し分け
  - ゲスト: `Login`
  - ログイン済み: `通知`, `予定作成` または詳細画面の文脈アクション
- モバイルでは縦積み。ゲスト時は `Login` の単独CTA、ログイン済みは複数CTAを折り返し可能にする。
- ゲストに `予定作成` をグローバルCTAとして出さない。保存・参加・作成の意図が画面内で発生した時点で、inline messageまたは `AuthRequiredNote` からログインへ接続する。
...
1. Featured events
2. Guest value banner
3. Public gym/event shortcuts
4. Topic rail
5. Public gym/event digest
6. Gym/event section
7. Gym/event cards
...
1. Member dashboard
2. Authenticated composer
3. Personal shortcuts
4. Today session digest
5. Featured events
6. Member/public feed header
7. Feed tabs
8. Feed cards
...
- body: `ジムとイベントを見てから、保存や参加が必要になったタイミングでログインできます。`
- CTA: `保存を始める`, `ログイン`

### 4.3 Member dashboard / Composer

目的: ログイン後は閲覧ではなく、保存ジム、参加予定、記録、作成を起点にすぐ行動できる状態にする。

Member dashboard:

- eyebrow: `Member home`
- title: `今日のセッション管理`
- body: `保存したジム、参加予定、記録を起点に、次の行動をすぐ作れます。`
- stats: `今週の予定`, `保存ジム`, `記録`
- links: `次の予定`, `保存ジム`, `最近の記録`
- CTA: `予定作成`, `記録追加`
