...
const instagramPostSourceLimit = parsePositiveInt(process.env.ZAC_INSTAGRAM_POST_SOURCE_LIMIT, 25);
...
try {
  ...
  const run = {
    ...
    operatorChecklist: [
      "Run the Instagram browser roller first. It may inspect only approved official Instagram sources in a logged-in browser session. Use the freshness lane for the latest visible posts/reels; when those are already known or the account has no observations yet, use the bounded backfill lane to scroll for the next unknown posts. Stop at the configured per-source limit, visible-post scan limit, or 60-day lookback.",
